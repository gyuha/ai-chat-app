...
/init 이후 아래 내용 붙여 넣기
```
추가로 파일에 gsd와 ui-ux-pro-max 스킬을 사용 할 때, 안내 메시지와 문서를 한글로 작성(설명)하도록 지침에 추가 해 주세요.
추가로 gsd에서 ui 관련된 작업을 할 때, ui-ux-pro-max-skill을 함께 사용 하도록 지침에 추가 해 주세요.
```
...
## 작업 지침
- GSD 관련 안내 메시지, 질문, 요약, 생성 문서(PROJECT.md, REQUIREMENTS.md, ROADMAP.md, UI-SPEC.md, PLAN.md, SUMMARY.md 등)는 한국어로 작성한다.
- `ui-ux-pro-max` 스킬을 사용할 때도 설명, 디자인 근거, 추천안, 생성 문서는 한국어로 작성한다.
- 코드, 명령어, 파일 경로, REQ-ID, phase 번호, API/라이브러리 이름 같은 식별자는 원문 표기를 유지한다.
