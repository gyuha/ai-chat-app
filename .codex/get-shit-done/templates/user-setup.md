...
Template for `.planning/phases/XX-name/{phase}-USER-SETUP.md` - the agent가 자동화할 수 없는 사람 손 작업을 기록하는 템플릿입니다.

**Purpose:** 계정 생성, 대시보드 설정, secret 조회처럼 사람이 직접 해야 하는 설정만 기록합니다. the agent가 자동화할 수 있는 것은 최대한 자동화하고, 남는 수동 작업만 이 파일에 남깁니다.
