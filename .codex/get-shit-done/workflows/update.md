<purpose>
한국어 우선 안내: 이 워크플로 자산은 `update` 흐름을 한국어 기준으로 먼저 읽을 수 있게 정리합니다. 아래 영문 원문은 upstream 동기화와 세부 의미 보존을 위해 함께 유지합니다.
...
```bash
npm view get-shit-done-ko version 2>/dev/null
```
...
To update manually: `npx get-shit-done-ko --global`
```
...
global: `/Users/gyuha/workspace/ai-chat-app/.codex/`, `~/.config/opencode/`, `~/.opencode/`, `~/.gemini/`, or `~/.codex/`
...
**If LOCAL install:**
```bash
npx -y get-shit-done-ko@latest "$RUNTIME_FLAG" --local
```

**If GLOBAL install:**
```bash
npx -y get-shit-done-ko@latest "$RUNTIME_FLAG" --global
```

**If UNKNOWN install:**
```bash
npx -y get-shit-done-ko@latest --claude --global
```
