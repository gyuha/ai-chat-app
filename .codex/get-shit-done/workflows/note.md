<purpose>
한국어 우선 안내: 이 워크플로 자산은 `note` 흐름을 한국어 기준으로 먼저 읽을 수 있게 정리합니다. 아래 영문 원문은 upstream 동기화와 세부 의미 보존을 위해 함께 유지합니다.
...
- **Global scope**: `/Users/gyuha/workspace/ai-chat-app/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/gyuha/workspace/ai-chat-app/.codex/notes/`)
...
2. Glob `/Users/gyuha/workspace/ai-chat-app/.codex/notes/*.md` (if directory exists) — global notes
...
Global (/Users/gyuha/workspace/ai-chat-app/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/gyuha/workspace/ai-chat-app/.codex/notes/` — works in any directory
