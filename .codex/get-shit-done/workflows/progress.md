<purpose>
한국어 우선 안내: 이 워크플로 자산은 `progress` 흐름을 한국어 기준으로 먼저 읽을 수 있게 정리합니다. 아래 영문 원문은 upstream 동기화와 세부 의미 보존을 위해 함께 유지합니다.
...
```bash
INIT=$(node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" init progress)
...
- `ROADMAP=$(node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap analyze)`
- `STATE=$(node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
DEBT=$(node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
- `$gsd-list-phase-assumptions {phase}` — the agent의 가정 확인
...
- `$gsd-list-phase-assumptions {phase} ${GSD_WS}` — the agent의 가정 확인
...
```bash
NEXT_PHASE_SECTION=$(node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
