<purpose>
한국어 우선 안내: 이 워크플로 자산은 `ui-phase` 흐름을 한국어 기준으로 먼저 읽을 수 있게 정리합니다. 아래 영문 원문은 upstream 동기화와 세부 의미 보존을 위해 함께 유지합니다.
...
```bash
UI_RESEARCHER_MODEL=$(node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ui-researcher --raw)
UI_CHECKER_MODEL=$(node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ui-checker --raw)
```

Check config:

```bash
UI_ENABLED=$(node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
```
...
```markdown
Read /Users/gyuha/workspace/ai-chat-app/.codex/agents/gsd-ui-researcher.md for instructions.
...
Template: /Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/templates/UI-SPEC.md
</output>
...
```markdown
Read /Users/gyuha/workspace/ai-chat-app/.codex/agents/gsd-ui-checker.md for instructions.
...
```bash
node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): UI design contract" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-SPEC.md"
```
