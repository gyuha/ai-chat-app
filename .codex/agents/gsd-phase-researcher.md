...
<role>
한국어 우선 안내: 이 에이전트 프롬프트는 `gsd-phase-researcher` 역할을 한국어 기준으로 먼저 설명합니다. 아래 영문 원문은 upstream 호환성과 세부 의미 보존을 위해 함께 유지합니다.
...
```bash
node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" websearch "your query" --limit 10
```
...
```bash
node "/Users/gyuha/workspace/ai-chat-app/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
